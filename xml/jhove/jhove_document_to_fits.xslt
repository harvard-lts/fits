            ...
            <metadata>
                <document>
                    ...
                    <title>
                        <xsl:choose>
                            <xsl:when
                                    test="//property[name='Info']/values/property['Title']/values/value and //property[name='Info']/values/property['Title']/values/value != '&lt;May be encrypted&gt;'">
                                <xsl:value-of
                                        select="//property[name='Info']/values/property[name='Title']/values/value"/>
                            </xsl:when>
                            <xsl:otherwise>
                    ...
                    <author>
                        <xsl:choose>
                            <xsl:when
                                    test="//property[name='Author']/values/value and //property[name='Author']/values/value != '&lt;May be encrypted&gt;'">
                    ...
                    <pageCount>
                        <xsl:variable name="pageCount"
                                      select="count(//property[name='Pages']/values/property[name='Page'])"/>
                    ...
                    <graphicsCount>
                        <xsl:variable name="graphicsCount"
                                      select="count(//property[name='Images']/values/property[name='Image'])"/>
                    ...
                    <xsl:variable name="fontNames" as="element()*">
                        <xsl:for-each select="//property[name='Fonts']//property[name='FontName']/values/value[1]">
                            <xsl:element name="anElement">
                                ...
                                <xsl:value-of
                                        select="if (contains($fontText,'+')) then replace($fontText, '[A-Z]+\+', '') else $fontText"/>
                            </xsl:element>
                        </xsl:for-each>
                    </xsl:variable>
